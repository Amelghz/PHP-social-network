...
class Post
{
			...
			if($this->error == ""){
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						add_notification($_SESSION['myesprims_userid'],"comment",$mypost);
					}
	...
	public function i_own_post($postid,$myesprims_userid)
	{
		...
		$result = $DB->read($query);
  		...
  		if(is_array($result)){
  			...
  			if($result[0]['userid'] == $myesprims_userid){
	...
	public function like_post($id,$type,$myesprims_userid){
			...
			if(is_array($result)){
				...
				if(!in_array($myesprims_userid, $user_ids)){

					$arr["userid"] = $myesprims_userid;
					...
					if($type != "user"){
						...
						//add notification
						add_notification($_SESSION['myesprims_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($myesprims_userid, $user_ids);
					unset($likes[$key]);
