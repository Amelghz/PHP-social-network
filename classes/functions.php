...
function i_own_content($row){
	...
	$myid = $_SESSION['myesprims_userid'];
...
//$test =($myid == $one_post['userid']);
			//if($myid == $one_post['userid']){
			//	$test=true;
              
			//}else{$test=false;}
			$myid == $one_post['userid']??= 'default value';
			...
			if(is_array($tagged_user)){
				...
				if($new_post_text != ""){
					...
					if(!in_array($tagged_user['tag_name'], $old_tags)){
						add_notification($_SESSION['myesprims_userid'],"tag",$mypost,$tagged_user['userid']);
					}
				}else{
...
function notification_seen($id)
{
	...
	$userid = $_SESSION['myesprims_userid'];
	$DB = new Database();
...
if(isset($_SESSION['myesprims_userid'])){
	set_online($_SESSION['myesprims_userid']);
}
...
function check_messages(){
	...
	$DB = new Database();
	$me = esc($_SESSION['myesprims_userid']);
